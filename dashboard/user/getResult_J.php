...
class PDF extends FPDF {
    ...
    function calAv($conn){
        ...
        while ($row = $result->fetch_assoc()) {
            $total = $row['firstTest'] + $row['secondTest'] + $row['assignment'] + $row['project'] + $row['exam'];
    ...
    function getPosition($conn){
        ...
        global $term;
        global $admno;
         $sql17 = " SELECT DISTINCT admno FROM published_result WHERE class = '".$class."' AND term = '".$term."' ";
         ...
         while ($row17 = mysqli_fetch_array($result17)) {
             ...
             while ($row18 = mysqli_fetch_array($result18)) {
                $avg_current = $avg_current + (float)$row18['firstTest'] + (float)$row18['secondTest'] + (float)$row18['assignment'] + (float)$row18['project'] + (float)$row18['exam'];
                 $counter++;
             }
    ...
    function header(){
        ...
        $this->cell(200,5,'DIVINE FAVOUR INTERNATIONAL SCHOOLS AWAKA',0,0,'C');
        $this->Ln(1);
        /*$this->Image('sign/traco.png',10,10,10);*/
        $this->SetFont('Arial','',10);
        $this->cell(70,20,$this->Image('sign/dfis_logo_p.png',10,10,22),0,0,'C');
        //$this->cell(50,20,$this->Image('sign/cof.png',180,10,24),0,0,'C');
        $this->cell(50,20,$this->Image('file/profile/'.$_SESSION['sd']['photo'],180,10,24),0,0,'C');
        $this->Ln(6);
        $this->cell(190,5,'OPPOSITE HOLY TRINITY CATHOLIC CHURCH ',0,0,'C');
        $this->Ln(4);
        $this->cell(190,5,'AWAKA OWERRI NORTH',0,0,'C');
        ...
        $this->cell(80,6,$_SESSION['sd']['surname'].' '.$_SESSION['sd']['lastname'].' '.$_SESSION['sd']['middlename'],1,0,'L');
        $this->SetFont('Times','B',10);
        $this->cell(25,6,'Admission No',1,0,'L');
        $this->SetFont('Times','',10);
        $this->cell(77,6,$_SESSION['sd']['admno'],1,0,'L');
        /*$this->SetFont('Times','B',10);
        $this->cell(12,6,'Name',1,0,'L');
        $this->SetFont('Times','',10);
        ...
        $this->cell(25,6,'Admission No',1,0,'L');
        $this->SetFont('Times','',10);
        $this->cell(55,6,$_SESSION['sd']['admno'],1,0,'L');
        $this->SetFont('Times','B',10);
        $this->cell(12,6,'Class',1,0,'L');
        $this->SetFont('Times','',10);
        $this->cell(20,6,$class,1,0,'L');*/
        ...
        $this->SetFont('Times','',10);
        $this->cell(55,6,$term,1,0,'L');
        $this->SetFont('Times','B',10);
        $this->cell(25,6,'Avarage',1,0,'L');
        $this->SetFont('Times','',10);
        $this->cell(44,6,$this->calAv($conn),1,0,'L');
        /*WITH POSTION*/
        /*$this->SetFont('Times','B',10);
        $this->cell(25,6,'Term',1,0,'L');
        $this->SetFont('Times','',10);
        $this->cell(50,6,$term,1,0,'L');
        $this->SetFont('Times','B',10);
        $this->cell(25,6,'Avarage',1,0,'L');
        $this->SetFont('Times','',10);
        $this->cell(40,6,$this->calAv($conn),1,0,'L');
        $this->SetFont('Times','B',10);
    ...
    function footer(){
        ...
        $this->cell(10,10,$this->Image('sign/sign dfis.png',55,270,40),0,0,'C');
    ...
    function headerTable(){
        ...
        $this->cell(5,5,'S/N',1,0,'C');
        $this->cell(55,5,'Subject',1,0,'C');
        $this->cell(15,5,'1st Test',1,0,'C');
        $this->cell(15,5,'2nd Test',1,0,'C');
        $this->cell(20,5,'Assignment',1,0,'C');
        $this->cell(17,5,'Project',1,0,'C');
        $this->cell(15,5,'Exam',1,0,'C');
    ...
    function viewTable($conn){
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $total = (float)$row['firstTest'] + (float)$row['secondTest'] + (float)$row['assignment'] + (float)$row['project'] + (float)$row['exam'];
            $this->cell(5,6,$cnt++,1,0,'C');
            $this->cell(55,6,$row['subject'],1,0,'L');
            $this->cell(15,6,(float)$row['firstTest'],1,0,'C');
            $this->cell(15,6,(float)$row['secondTest'],1,0,'C');
            $this->cell(20,6,(float)$row['assignment'],1,0,'C');
            $this->cell(17,6,(float)$row['project'],1,0,'C');
            $this->cell(15,6,(float)$row['exam'],1,0,'C');
            $this->cell(15,6,$total,1,0,'C');
            $this->cell(15,6,$passG = $this->getGrade($total),1,0,'C');
            $this->getPass($passG);
            ...
            $this->cell(97,6,'PRINCIPAL REMARK',1,0,'C');
            $this->Ln();
            ...
            $this->Ln();
            $this->SetFont('Times','',8);
            $this->cell(64,6,'Acess Bank',1,0,'C');
            $this->cell(65,6,'Divine Favour International Schools Awaka',1,0,'C');
            $this->cell(65,6,'1469540241',1,0,'C');
...
$pdf->AliasNbPages();
$pdf->AddPage('P','A4',0);
$pdf->Image('sign/dfis bw.png',10,10,189);
$pdf->headerTable();
$pdf->viewTable($conn);
$pdf->updateCard($conn);
$pdf->Output();
